...
public class DonuPatch {
    ...
    public static class PreBattleActionFix{
        ...
        public static void PreFix(Donu __instance){
            __instance.drawX = (float) Settings.WIDTH * 0.75F + 70 * Settings.xScale;
            __instance.drawY = AbstractDungeon.floorY + 10 * Settings.yScale;
    ...
    @SpirePatch(clz = Donu.class, method = "getMove")
    public static class getMove{
        @SpirePrefixPatch
        public static SpireReturn<Void> PreFix01(Donu __instance){
    ...
    public static class damage{
        @SpireInsertPatch(loc = 88, localvars = {"info"})
        public static SpireReturn<Void> PreFix(Donu __instance, DamageInfo ___info){
            if (__instance.currentHealth <= 0 && !__instance.halfDead) {
                ...
                for (AbstractMonster m : (AbstractDungeon.getMonsters()).monsters) {
                    if (m.id.equals("Deca") && !m.halfDead)
                        allDead = false;
                }
                if (!allDead) {
                    if (__instance.nextMove != 114) {
    ...
    @SpirePatch(clz = Donu.class, method = "die")
